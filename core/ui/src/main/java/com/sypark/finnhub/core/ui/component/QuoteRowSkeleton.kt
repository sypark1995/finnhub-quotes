...
fun QuoteRowSkeleton(modifier: Modifier = Modifier) {
    Row(
        ...
        Box(
            modifier = Modifier
                .size(40.dp)
                .background(color = MaterialTheme.colorScheme.surfaceVariant, shape = ShapeSmall)
                .shimmerEffect(),
        )
        Column(
            modifier = Modifier
            ...
            Box(Modifier.width(64.dp).height(16.dp).background(MaterialTheme.colorScheme.surfaceVariant, ShapeExtraSmall).shimmerEffect())
            Box(Modifier.width(100.dp).height(14.dp).background(MaterialTheme.colorScheme.surfaceVariant, ShapeExtraSmall).shimmerEffect())
        }
        Column(horizontalAlignment = Alignment.End, verticalArrangement = Arrangement.spacedBy(Spacing.space1)) {
            Box(Modifier.width(72.dp).height(18.dp).background(MaterialTheme.colorScheme.surfaceVariant, ShapeExtraSmall).shimmerEffect())
            Box(Modifier.width(48.dp).height(14.dp).background(MaterialTheme.colorScheme.surfaceVariant, ShapeExtraSmall).shimmerEffect())
        }
    }
}
